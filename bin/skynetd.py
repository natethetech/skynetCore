...
def HVAC_init():
    ...
    for relayCounter in range(len(pinList)):
        ...
        logger.debug("Pin %s" % pinList[relayCounter])
...
def poll_1wire_temps():
    ...
    if len(device_folder) != 3:
	logger.error("%s devices found" % len(device_folder))
        logger.error("1WIRE ERROR: SENSOR COUNT WRONG! RUNNING RESET ROUTINE!")
        oneWirePowerCycle()
    else:
        logger.debug("Located sensors OK")
        device_file[0] = device_folder[0] + '/w1_slave'
        device_file[1] = device_folder[1] + '/w1_slave'
        device_file[2] = device_folder[2] + '/w1_slave'
